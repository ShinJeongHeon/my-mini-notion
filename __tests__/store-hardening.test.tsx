...
// ── 프로필 행 생성 경합 (select→insert race) ────────────────────────────

test("프로필 행 생성 경합에서 진 탭도 승자의 행을 읽어 반영한다", async () => {
  state.session = googleSession;
  state.profileRow = null; // 최초 로그인 — select 시점엔 행이 없다
  // upsert 시점에는 다른 탭이 먼저 행을 만든 상황(DO NOTHING).
  state.profileUpsertConflictRow = {
    name: "경합승자",
    image_path: null,
    introduction: null,
  };
  ...
  // 진 탭이 조용히 폴백에 머물지 않고 승자의 별명을 읽어와야 한다.
  await waitFor(() =>
    expect(screen.getByTestId("display-name").textContent).toBe("경합승자")
  );
});
