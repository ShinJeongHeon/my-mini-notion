...
// F2 회귀: 별명도 자기소개처럼 늦게 도착한 DB 값으로 재동기화되어야 한다 —
// 아니면 자기소개만 고쳐 저장해도 stale 별명이 DB를 덮어쓴다.
test("DB 별명이 늦게 도착해도 미입력 상태의 별명 입력란에 채워진다", async () => {
  state.profileRow = { name: "민수짱", image_path: null, introduction: null };
  renderMyPage();
  await waitFor(() => {
    const nick = screen.getByLabelText("별명") as HTMLInputElement;
    expect(nick.value).toBe("민수짱");
  });
});
