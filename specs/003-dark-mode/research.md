...
- **Rationale**: 번들 문서 `script.md` 확인 결과 `next/script`의
  ...
  번들 가이드 `01-app/02-guides/preventing-flash-before-hydration.md`의 "Themes"
  섹션이 이 패턴(루트 레이아웃 `<head>` 인라인 스크립트 + `suppressHydrationWarning`)
  을 공식 예시로 확인해 준다.
- **구현 중 수정(2026-07-22)**: 이 Next.js 버전은 서버 컴포넌트가 import하는 모듈의
  훅 import를 컴파일 단계에서 거부하므로, 레이아웃이 `lib/theme.ts`("use client")를
  직접 import할 수 없다. 스크립트 주입은 클라이언트 컴포넌트
  `components/ThemeScript.tsx`가 담당한다(SSR 시에도 `<head>`에 렌더되어 no-FOUC
  동일 — Playwright 검증 통과). plan.md "구현 중 편차" 참조.
