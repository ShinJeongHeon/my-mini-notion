...
- **Decision**: `window.matchMedia("(prefers-color-scheme: dark)")`로 해석.
  ...
  결과를 무시한다(리스너 콜백에서 저장값 존재 시 no-op).
...
- **Decision**: 전용 키 `mini-notion-theme`에 원시 문자열 `"light"`/`"dark"`
  저장. 읽을 때 두 값 외에는 전부 "미설정"으로 간주(검증). 모든
  `localStorage` 접근은 try/catch로 감싸 저장 차단 환경에서도 토글 자체는
  동작하게 한다(이 경우 재방문 시 초기 테마 복귀 — spec Edge Case 허용).
- **Rationale**: 기존 `mini-notion-v1`(posts/nickname/avatar, DESIGN.md §5.6)은
  ...
  같은 패턴(전용 키 + 원시 값 + try/catch)을 쓰는 것으로 충족한다.
...
- **Decision**: **원시 램프(`--gray-*` 등)는 불변**, 시맨틱 토큰만 다크 값으로
  오버라이드한다. 시맨틱 계층을 우회한 사용처 6곳은 이번에 토큰화한다:

  | 사용처 (DESIGN.md 근거) | 현행 | 조치 |
  |---|---|---|
  | `.brand-chip` bg/color (§2.7.8) | `--gray-900` / `#fff` | 신규 `--surface-inverse`(기존 토큰 활용)/`--text-on-inverse`(신규)로 교체 |
  | `.detail-char-count` bg/color (§2.7.16) | `--gray-900` / `--text-on-accent` | 위와 동일한 inverse 토큰 쌍으로 교체 |
  | `.badge` bg (§2.3) | `--gray-100` | `--surface-hover`(동일 최종값)로 시맨틱 교체 |
  | `.mn-scroll` thumb (§2.7.15) | `--gray-200`/`--gray-300` | 신규 `--scrollbar-thumb`/`--scrollbar-thumb-hover` |
  | `.detail-delete-btn:hover` bg (§2.7.4) | `#fdeae9` 하드코딩 | 신규 `--status-danger-soft` |
  | `::selection` (§7) | `--accent-soft` | 토큰 유지(다크 블록에서 값만 오버라이드) |

  구체적 다크 값 표(약 30개)와 대비 검증은 `contracts/theme-contract.md`에
