...
- **스키마:** `profile { id uuid PK, created_at, name text|null, user_id uuid FK→auth.users.id (unique, on delete cascade) }`. `user_id` unique 제약으로 1:1 보장. RLS 활성 + 본인 행(select/insert/update, `auth.uid() = user_id`)만 허용.
