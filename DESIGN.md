...
- **AppState:** `{ dataLoaded, authLoaded, posts: Post[], nickname: string|null, avatar: string|null, account: Account|null }`. `Account = { id, email, name, avatarUrl }`(구글 세션에서 매핑, §5.5; `id`는 `auth.users.id` — profile 연동 키, §5.9).
- **파생값(외부 API):** `loaded = dataLoaded && authLoaded`, `loggedIn = !!account`, `displayName = nickname || account.name || "김민수"`, `email = account.email ?? ""`, `avatar = 업로드 아바타 ?? account.avatarUrl ?? null`(구글 프로필 사진이 기본값, 업로드가 우선).
- **localStorage 키:** `"mini-notion-v1"`.
- **저장 스키마:** `JSON.stringify({ posts, nickname, avatar })` — **로그인 상태는 저장하지 않음**(세션은 supabase-js가 자체 관리). `dataLoaded` 이후 모든 변경 시 저장(`useEffect([state])`).
- **로드:** 최초 마운트에서 `localStorage.getItem(KEY)` → 있으면 파싱(`posts` 배열 검증, `nickname/avatar` 없으면 null), 없거나 파싱 실패 시 `seed()`로 시드. 과거 스키마의 `loggedIn` 필드는 무시.
...
- **스키마:** `profile { id uuid PK, created_at, name text|null, introduction text|null, user_id uuid FK→auth.users.id (unique, on delete cascade) }`. `user_id` unique 제약으로 1:1 보장. RLS 활성 + 본인 행(select/insert/update, `auth.uid() = user_id`)만 허용. `introduction`은 자기소개 저장용 선택 컬럼(현재 UI 미사용 — `specs/003-profile-introduction` 예정).
...
- **클라이언트 로드(`syncProfile`):** `onAuthStateChange`의 `INITIAL_SESSION`/`SIGNED_IN`에서 다음 틱으로 미뤄(auth 락 교착 회피) `profile.name`을 조회해 `nickname` 상태에 반영 — **DB가 별명의 원본**. 행이 없으면(트리거 도입 이전 가입자) 구글 이름을 초기값으로 insert.
...
| `lib/store.tsx` | 상태·영속화·시드·날짜포맷·상수·구글 세션 매핑 | §5, §6.6 |
