...
- **saveProfile(async):** `{ name, introduction }`을 받아 name은 `(name||"").trim() || null`(빈 값이면 null → displayName은 다시 구글 이름/"김민수")로 정규화해 로컬 상태에 즉시 반영한 뒤, Supabase `profile.name`+`profile.introduction`을 **단일 update**로 저장(§5.9 — 별명·자기소개가 항상 함께, 부분 저장 없음). 성공 여부 `Promise<boolean>` 반환 — 마이페이지가 실패 알림에 사용(§4.4).
